...
const Step6: React.FC<Props> = ({ prevStep, formData, update }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLButtonElement>) => {
    ...
    const guncellenmisFormData = {
      ...formData,
      karbonAyakIzi: karbonAyakIziDegeri,
    };
    ...
    try {
      const data = await updateCarbonDetails(
        formDataId,
        guncellenmisFormData,
        token,
      );
  ...
  return (
    <div className="p-6 rounded-xl shadow-md space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        <p>
          <strong>Lokasyon:</strong> {formData.firma?.lokasyon || "N/A"}
        </p>
        <p>
          <strong>Sektör:</strong> {formData.firma?.sektor || "N/A"}
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        <p>
          <strong>Üretilen Ürün:</strong> {formData.firma?.urun || "N/A"}
        </p>
        <p>
          <strong>Üretim Miktarı:</strong> {formData.firma?.miktar || "N/A"}{" "}
          {formData.firma?.birim || "N/A"}
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        <p>
          <strong>Elektrik:</strong> {formData.enerji?.elektrikMiktar}{" "}
          {formData.enerji?.elektrikBirim} ({formData.enerji?.elektrikDonem})
        </p>
        <p>
          <strong>Doğalgaz:</strong> {formData.enerji?.dogalgazMiktar}{" "}
          {formData.enerji?.dogalgazBirim} ({formData.enerji?.dogalgazDonem})
        </p>
        <p>
          <strong>Kömür:</strong> {formData.enerji?.komurMiktar || "N/A"}{" "}
          {formData.enerji?.komurBirim || ""}
        </p>
        <p>
          <strong>Elektrik Kaynağı:</strong>{" "}
          {formData.enerji?.elektrikKaynak || "N/A"}
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        {(formData.yakitHammadde?.yakitlar || []).map(
          (yakit: any, i: number) => (
            <p key={i}>
              <strong>
                {yakit.tip.charAt(0).toUpperCase() + yakit.tip.slice(1)}:
              </strong>{" "}
              {yakit.miktar} {yakit.birim} ({yakit.donem})
            </p>
          ),
        )}
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        {(formData.yakitHammadde?.hammaddeler || []).map(
          (madde: any, i: number) => (
            <p key={i}>
              <strong>
                {madde.ad.charAt(0).toUpperCase() + madde.ad.slice(1)}:
              </strong>{" "}
              {madde.miktar} {madde.birim} ({madde.tedarik})
            </p>
          ),
        )}
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        <p>
          <strong>Süreç Tipi:</strong> {formData.emisyon?.surecTipi || "N/A"}
        </p>
        <p>
          <strong>Emisyon Faktörü:</strong>{" "}
          {formData.emisyon?.emisyonFaktoru || "N/A"}
        </p>
        <p>
          <strong>CO₂:</strong> {formData.emisyon?.co2} kg
        </p>
        <p>
          <strong>CH₄:</strong> {formData.emisyon?.ch4} kg
        </p>
        <p>
          <strong>N₂O:</strong> {formData.emisyon?.n2o} kg
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-6">
        ...
        <p>
          ...
          {formData.atikGeriDonusum?.atikTipi || "N/A"}
        </p>
        <p>
          <strong>Atık Miktarı:</strong>{" "}
          {formData.atikGeriDonusum?.atikMiktari || "N/A"}
        </p>
        <p>
          <strong>Geri Dönüşüm Oranı:</strong>{" "}
          {formData.atikGeriDonusum?.geriDonusumOrani || "N/A"}%
        </p>
      </div>
